...
public class ChaptersDBFragment extends Fragment {
    ...
    SharedPreferences ChapterDetail;

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View RootView = inflater.inflate(R.layout.frag_chapterdb, container, false);

        mDatabasehelper = new DatabaseHelper(getActivity());

        getActivity().setTitle("Chapters In DB");

        ChapterDetail = getActivity().getSharedPreferences("chapterCreationDetails", Context.MODE_PRIVATE);

        listView = (ExpandableListView) RootView.findViewById(R.id.expListsC);
        ...
        listView.setOnChildClickListener(new ExpandableListView.OnChildClickListener(){
            ...
            public boolean onChildClick(ExpandableListView parent, View v, int groupPosition, int childPosition, long id) {
                SharedPreferences.Editor editor = ChapterDetail.edit();
                editor.putString("chaptName", listDataHeader.get(groupPosition));
                editor.commit();
...
//                        Toast.makeText(getActivity(), "User ID: " + String.valueOf(groupPosition) + " Clicked on: " + listHash.get(listDataHeader.get(groupPosition)).get(childPosition), Toast.LENGTH_SHORT).show();
                    ...
                    Intent intent = new Intent(getActivity(), ChapterEdit.class);
                    startActivity(intent);
//                        Toast.makeText(getActivity(), "User ID: " + String.valueOf(groupPosition) + " Clicked on: " + listHash.get(listDataHeader.get(groupPosition)).get(childPosition), Toast.LENGTH_SHORT).show();

                }else if (listHash.get(listDataHeader.get(groupPosition)).get(childPosition) == "Delete"){
                    Intent intent = new Intent(getActivity(), ChapterDelete.class);
                    startActivity(intent);
//                        Toast.makeText(getActivity(), "User ID: " + String.valueOf(groupPosition) + " Clicked on: " + listHash.get(listDataHeader.get(groupPosition)).get(childPosition), Toast.LENGTH_SHORT).show();
        ...
        createnewu = (TextView) RootView.findViewById(R.id.createnewC);
    ...
    private void initData() {
        ...
        Cursor cursor = mDatabasehelper.allChapterData();
...
//        System.out.println(cursor.getString(1));
        ...
        int counter = 0;
        try {
            listDataHeader.add(cursor.getString(cursor.getColumnIndex("chapterName")));
